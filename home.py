...
if st.session_state.page == "home":
    ...
    st.write("- At first you have to sign up and then you can order.")
    st.write("")
    st.divider()
        ...
        if not (username and password):
            ...
            for u in st.session_state.users:
                if u.name == username and u.password == password:
                    userMatch = True
                    st.success(f"Sign in completed successfully.")
    ...
    col1, col2 = st.columns(2)
    with col1:
        with st.expander("View today's menu", expanded=True) :
